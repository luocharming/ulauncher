...
func (m *runnerManager) StartInternalRunner() {
	if m.HaveInternalInstance && !m.IsInternalInstanceStarted {
		...
		if err == nil {
			...
			if runner != nil && !internal.EnableAutoStart {
				runner.Start()
...
func (m *runnerManager) StartAutoStartRunner() {
	instanceList := InstanceManager.List()
	for _, instance := range instanceList {
		if instance.EnableAutoStart {
			runner := m.GetRunnerById(instance.CID)
			if runner != nil {
				runner.Start()
				logger.Zap.Infof("自动启动实例 %s", instance.Name)
			}
		}
	}
}

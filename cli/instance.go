...
var instanceAddCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		prompt2 := &survey.MultiSelect{
			Message:  "实例设置:",
			PageSize: 10,
			Options:  []string{"渲染控制", "多用户同时操作", "自动启动实例", "共享实例", "故障恢复", "自动启停", "使用WebRTC中继", "根据视口大小调整分辨率", "隐藏控制UI", "无操作关闭连接"},
		...
		_ = survey.AskOne(prompt6, &instance.SceneId)
...
var instanceModCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		prompt7 := &survey.Input{
			Message: "默认场景:",
			Default: instance.SceneId,
		}
		_ = survey.AskOne(prompt7, &instance.SceneId)
...
func setSelected(instance *model.ClientInstance, selected []string) {
	...
	instance.EnableAutoStart = false
	instance.EnableSharedInstance = false
	...
	for _, s := range selected {
		if "渲染控制" == s {
			...
			instance.EnableAutoStart = true
		} else if "共享实例" == s {
			instance.EnableSharedInstance = true
		} else if "故障恢复" == s {
			instance.FaultRecover = true
		} else if "自动启停" == s {
			instance.AutoControl = true
...
func reverseSelected(instance *model.ClientInstance) []string {
	...
	if instance.EnableSharedInstance {
		selected = append(selected, "共享实例")
	}
	if instance.EnableAutoStart {
		selected = append(selected, "自动启动实例")
	}
